...
/** Static shell: level codes + module/lesson counts, no session data. Safe to render before auth resolves. */
export async function getCurriculumOverview() {
  if (!sql) return { levels: [] as { code: string; name: string; moduleCount: number; lessonCount: number }[] };
  const rows = (await sql`
    ...
    LEFT JOIN curriculum_submodules s ON s.module_id = m.id
    LEFT JOIN curriculum_lessons l ON l.submodule_id = s.id
    GROUP BY lv.id, lv.code, lv.name, lv.sort_order
    ORDER BY lv.sort_order, lv.code
  `) as { code: string; name: string; sort_order: number; module_count: number; lesson_count: number }[];
  return {
    levels: rows.map((r) => ({
      code: r.code,
      name: r.name,
...
/** Full curriculum tree + flat path (pathSteps, progress, session-aware fields). */
export async function getCurriculumData(pathOnly: boolean) {
  if (!sql) {
    return pathOnly
      ? { levels: [], pathSteps: [], totalEstimatedMinutes: 0, pathProgressPercent: 0, completedLessonIds: [] }
      : { levels: [] };
  }
