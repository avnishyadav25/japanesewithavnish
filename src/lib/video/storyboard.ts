...
/**
 * The recall round: show, pause, reveal, pronounce — over everything just taught.
 *
 * Shared by vocabulary, kanji and grammar rather than written three times, because the format IS
 * the same in each: a prompt in Japanese, a gap to answer in, then the meaning. Only what fills
 * `prompt` and `answer` differs — a word and its meaning, a character and its meaning, a pattern
 * and what it does.
 *
 * Uses `quiz_question`, which has been fully built and rendered since the first version and
 * generated by nothing until now.
 */
function buildRecallRound(
  pairs: { prompt: string; answer: string; reading?: string }[],
  config: GenerateStoryboardConfig,
  lang: NarrationLang,
  pacing: PacingConfig
  ...
  const asked = recall.maxQuestions ? pairs.slice(0, recall.maxQuestions) : pairs;
  if (asked.length === 0) return { scenes, blanks };
  ...
  scenes.push({
    ...
    visual: {
      ...
      subtitle: `${asked.length} to recall — answer before it appears`,
    },
  });
  blanks.push({
    id: introId,
    hint: `Introduce a recall round over ${asked.length} items just taught. Tell the viewer to answer out loud before it appears. Two short sentences.`,
    maxWords: 26,
  });

  asked.forEach((pair, i) => {
    ...
    scenes.push({
      ...
      minDurationSeconds: recall.thinkingSeconds + 3,
      durationSeconds: recall.thinkingSeconds + 3,
      transitionIn: { kind: "fade", durationSeconds: 0.25 },
      // Spoken as well as shown, so a listener can play along without watching.
      narration: [jaSegment(`${qBase}-ja`, pair.prompt, pair.reading, pacing, 0.2, false)],
      visual: {
        sceneType: "quiz_question",
        question: pair.prompt,
        // One option, revealed. The four-option layout the component also draws needs distractors
        // chosen carefully enough not to teach a wrong pairing — a different piece of work.
        options: [pair.answer],
        correctIndex: 0,
        thinkingSeconds: recall.thinkingSeconds,
        explanation: pair.reading && pair.reading !== pair.prompt ? pair.reading : undefined,
      },
    });
  });
...
function vocabularySkeleton(snapshot: ContentSnapshot, config: GenerateStoryboardConfig): Skeleton {
  ...
  if (config.recall && items.length > 1) {
    const recall = buildRecallRound(
      items.map((item) => ({ prompt: item.word, answer: item.meaning, reading: item.reading })),
      config,
      lang,
      pacing
    );
    scenes.push(...recall.scenes);
    blanks.push(...recall.blanks);
  }
...
function kanjiSkeleton(snapshot: ContentSnapshot, config: GenerateStoryboardConfig): Skeleton {
  ...
  if (config.recall && snapshot.items.length > 1) {
    const recall = buildRecallRound(
      snapshot.items.map((item) => ({
        prompt: str(item.data.character) ?? item.title,
        answer: str(item.data.meaning) ?? item.summary ?? "",
        // The on'yomi, cleaned of dictionary notation, so the voice does not read the dots aloud.
        reading: speakableReadings(strArray(item.data.onyomi)) ?? undefined,
      })),
      config,
      lang,
      pacing
...
function grammarSkeleton(snapshot: ContentSnapshot, config: GenerateStoryboardConfig): Skeleton {
  ...
  if (config.recall && snapshot.items.length > 1) {
    const recall = buildRecallRound(
      snapshot.items.map((item) => {
        const pattern = str(item.data.pattern) ?? item.title;
        return {
          prompt: pattern,
          answer: str(item.data.meaning) ?? item.summary ?? "",
          // Only the Japanese run is speakable — a pattern like "Verb volitional + と思います"
          // would otherwise have its English scaffolding read by the Japanese voice.
          reading: spokenGrammarPattern(item.data, pattern),
        };
      }),
      config,
      lang,
...
/**
 * Reading practice: the passage on screen, read aloud, then translated.
 *
 * `reading_passage` has had a component with auto-scroll since the first version and was emitted by
 * NOTHING — reading content fell through to genericSkeleton, which narrates a title and a summary
 * and never shows the passage at all.
 *
 * The passage lives in `meta.sentences`, each carrying japanese / romaji / translation. Sentences
 * are spoken one at a time rather than as one long clip: a learner following along needs the audio
 * to land on the line their eye is on, and a single 40-second clip cannot be paused per sentence.
 */
function readingSkeleton(snapshot: ContentSnapshot, config: GenerateStoryboardConfig): Skeleton {
  const lang = config.narrationLang;
  const pacing = config.pacing;
  const scenes: Scene[] = [];
  const blanks: BlankSlot[] = [];
  ...
  snapshot.items.forEach((item, i) => {
    const base = `sc-r${i + 1}`;
    const raw = (item.data.meta as { sentences?: unknown } | undefined)?.sentences;
    const sentences = (Array.isArray(raw) ? raw : [])
      .map((x) => x as Record<string, unknown>)
      .map((x) => ({ ja: str(x.japanese) ?? str(x.ja) ?? "", en: str(x.translation) ?? str(x.en) ?? "", romaji: str(x.romaji) }))
      .filter((x) => x.ja);

    if (sentences.length === 0) return;

    const introId = `${base}-lead`;
    const narration: NarrationSegment[] = [blankSegment(introId, lang, pacing)];
    const slots: { id: string; hint: string; weight?: number }[] = [
      { id: introId, hint: `Introduce a reading passage: "${item.title}". Say what it is about in one sentence, and tell the viewer to read along. Source: ${item.summary ?? ""}`.slice(0, 400), weight: 2 },
    ];
    const japanese: string[] = [];

    sentences.forEach((sentence, si) => {
      narration.push(jaSegment(`${base}-ja${si + 1}`, sentence.ja, undefined, pacing, si === 0 ? 0.4 : 0.2, false));
      japanese.push(sentence.ja);
    });
    ...
    scenes.push({
      ...
      visual: {
        sceneType: "reading_passage",
        title: item.title,
        passageJa: sentences.map((x) => x.ja).join(""),
        passageEn: sentences.map((x) => x.en).filter(Boolean).join(" "),
        // The component scrolls rather than shrinking to unreadable type. Past about four
        // sentences a passage cannot fit a frame at a legible size.
        scroll: sentences.length > 4,
      },
    });

    const recapId = `${base}-recap`;
    scenes.push({
      id: `${base}-after`,
      sceneType: "tip_callout",
      durationMode: "auto",
      durationSeconds: Math.max(5, pacing.secondsPerItem * 0.35),
      transitionIn: { kind: "slide", durationSeconds: 0.3 },
      narration: [blankSegment(recapId, lang, pacing)],
      visual: { sceneType: "tip_callout", heading: "What it said", body: sentences.map((x) => x.en).filter(Boolean).join(" ") || item.title },
    });
    slots.push({ id: recapId, hint: `Give the meaning of the passage just read, in your own words. It says: ${sentences.map((x) => x.en).join(" ").slice(0, 500)}`, weight: 3 });
...
/**
 * Listening practice: hear it, pause, see it.
 *
 * `listening_prompt` already had the whole mechanic — a countdown driven by `thinkingSeconds`, then
 * the answer and transcript revealed — and, like the quiz scene, was generated by nothing.
 *
 * The Japanese is spoken by the native voice with `pause = false`, because the gap here is the
 * scene's own thinking window rather than a shadowing pause; adding both would double it.
 */
function listeningSkeleton(snapshot: ContentSnapshot, config: GenerateStoryboardConfig): Skeleton {
  const lang = config.narrationLang;
  const pacing = config.pacing;
  const scenes: Scene[] = [];
  const blanks: BlankSlot[] = [];
  const thinking = config.recall?.thinkingSeconds ?? 3;

  snapshot.items.forEach((item, i) => {
    const base = `sc-li${i + 1}`;
    const clips = item.examples.slice(0, exampleBudget(pacing, item.examples.length) || item.examples.length);
    ...
    scenes.push({
      ...
      transitionIn: { kind: "fade", durationSeconds: 0.4 },
      narration: [blankSegment(introId, lang, pacing)],
      visual: { sceneType: "title_card", eyebrow: item.jlptLevel ? `JLPT ${item.jlptLevel}` : undefined, title: item.title, subtitle: `${clips.length} to listen to` },
    });
    blanks.push({ id: introId, hint: `Introduce listening practice: "${item.title}". Tell the viewer to listen and answer before the text appears. Two sentences.`, maxWords: 24 });

    clips.forEach((clip, ci) => {
      const qBase = `${base}-q${ci + 1}`;
      scenes.push({
        id: qBase,
        sceneType: "listening_prompt",
        sourceRef: item.postId ? { kind: "post", id: item.postId, url: item.url } : undefined,
        durationMode: "auto",
        minDurationSeconds: thinking + 4,
        durationSeconds: thinking + 4,
        transitionIn: { kind: "fade", durationSeconds: 0.3 },
        narration: [jaSegment(`${qBase}-ja`, clip.ja, exampleSpoken(clip), pacing, 0.3, false)],
        visual: {
          sceneType: "listening_prompt",
          prompt: "What did you hear?",
          transcriptJa: clip.ja,
          transcriptEn: clip.en,
...
export function buildSkeleton(snapshot: ContentSnapshot, config: GenerateStoryboardConfig): Skeleton {
  ...
  // Both of these used to fall through to genericSkeleton, which narrates a title and a summary —
...
/**
 * What each level means for the writing, as instructions rather than a label.
 *
 * The narration language axis is deliberate. A beginner needs the explanation in a language they
 * already have; an advanced learner is held back by having every phrase translated for them. The
 * pipeline already supports en/hi/hinglish/ja narration, so this is a budget and wording change
 * rather than new machinery.
 */
const LEVEL_GUIDANCE: Record<string, string> = {
  N5: "Audience: absolute beginners. Explain in the narration language throughout. Assume no grammar vocabulary — say \"the past form\" rather than \"the ta-form\". Say why a word is useful before what it means. Short sentences.",
  N4: "Audience: early learners. Explain fully but assume the basics are known. You may use common grammar terms without defining them.",
  N3: "Audience: intermediate. Be concise. Skip the scaffolding a beginner needs and spend the words on usage and nuance instead.",
  N2: "Audience: upper intermediate. Assume fluency with everyday grammar. Focus on register, nuance and where this differs from a similar expression. Do not translate what the on-screen text already shows.",
  N1: "Audience: advanced. Dense and precise. Comment on register, connotation and written-versus-spoken usage. Do not restate the meaning that is already on screen; add what a dictionary would not tell them.",
};
...
export function buildUserMessage(snapshot: ContentSnapshot, config: GenerateStoryboardConfig, slots: BlankSlot[]): string {
  ...
  return [
    ...
    // Level used to be a bare label the model could ignore. These are instructions it can act on:
    // how much scaffolding, and how much of the explanation stays in the narration language.
    snapshot.jlptLevel ? LEVEL_GUIDANCE[snapshot.jlptLevel.toUpperCase()] ?? "" : "",
    `Content type: ${kind}`,
