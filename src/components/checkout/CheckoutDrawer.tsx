...
export function CheckoutDrawer({ open, onClose, product, isPlan = false, currency = "INR" }: CheckoutDrawerProps) {
  useEffect(() => {
    if (!open) return;
      ...
      >
        <div className="flex items-center justify-between p-4 border-b border-[var(--divider)] shrink-0">
          <h2 id="checkout-drawer-title" className="font-heading text-lg font-bold text-charcoal">
            {isPlan ? "Premium Pass Checkout" : "Quick checkout"}
          </h2>
          <button
            type="button"
            onClick={onClose}
          ...
          <div className="p-4 pb-8 space-y-6">
            ...
            <div className="card p-4 bg-[var(--base)] border border-[var(--divider)]">
              <h3 className="font-heading text-sm font-bold text-charcoal mb-2">
                {isPlan ? "Premium Benefits" : "What you'll get"}
              </h3>
