...
// Sibling to src/app/api/admin/curriculum/lessons/[id]/blocks/[blockId]/route.ts,
// scoped to posts (content_blocks) instead of curriculum_lessons (lesson_blocks).

export async function PATCH(req: Request, { params }: { params: Promise<{ postId: string; blockId: string }> }) {
  const admin = await getAdminSession();
  if (!admin) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (!sql) return NextResponse.json({ error: "Database unavailable" }, { status: 503 });
  const { postId, blockId } = await params;
  const body = await req.json();

  const existingRows = await sql`SELECT block_type FROM content_blocks WHERE id = ${blockId} AND post_id = ${postId}`;
  ...
  const rows = await sql`
    UPDATE content_blocks SET
      ...
      updated_at = NOW()
    WHERE id = ${blockId} AND post_id = ${postId}
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ postId: string; blockId: string }> }) {
  const admin = await getAdminSession();
  if (!admin) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (!sql) return NextResponse.json({ error: "Database unavailable" }, { status: 503 });
  const { postId, blockId } = await params;
  const result = await sql`DELETE FROM content_blocks WHERE id = ${blockId} AND post_id = ${postId} RETURNING id`;
