...
export function ModuleEditFormClient({ moduleId, levelCode, initial }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/admin/curriculum/modules/${moduleId}`, {
        method: "PATCH",
  ...
  async function suggestWithAI() {
    ...
    try {
      const res = await fetch("/api/ai/curriculum/suggest-summary", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ entityType: "module", code, title, levelCode }),
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl space-y-4">
      ...
      <GenerateImageModal
        ...
        initialContext={{ title: title, topic: title, entityType: "module" }}
        initialPrompt=""
