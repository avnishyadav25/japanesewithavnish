...
type Props = {
  levelId: string;
  initial: { code: string; name: string; sort_order: number; summary: string; description: string; feature_image_url: string };
};

export function LevelEditFormClient({ levelId, initial }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/admin/curriculum/levels/${levelId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ code, name, sort_order, summary: summary || null, description: description || null, feature_image_url: feature_image_url.trim() || null }),
      });
      if (!res.ok) {
  ...
  async function suggestWithAI() {
    ...
    try {
      const res = await fetch("/api/ai/curriculum/suggest-summary", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ entityType: "level", code, name }),
      });
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl space-y-4">
      ...
      <div>
        ...
        <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full px-3 py-2 border border-[var(--divider)] rounded-bento" required />
      </div>
      <div>
      ...
      <div>
        ...
        <textarea value={summary} onChange={(e) => setSummary(e.target.value)} rows={2} className="w-full px-3 py-2 border border-[var(--divider)] rounded-bento" placeholder="Short blurb for cards/lists" />
      </div>
      <div>
      ...
      <GenerateImageModal
        ...
        initialContext={{ title: name, topic: name, entityType: "level" }}
        initialPrompt=""
