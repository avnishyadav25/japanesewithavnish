  ...
  if (sql) {
    ...
    const rows = filter
      ? await sql`
          ...
          FROM feedback WHERE status = ${filter}
      ...
      <div className="flex flex-wrap items-center gap-4 mb-6">
        ...
        <div className="flex gap-2">
          {["all", "new", "read", "replied"].map((s) => (
            <Link
              key={s}
              href={s === "all" ? "/admin/feedback" : `/admin/feedback?status=${s}`}
      ...
      {items.length > 0 ? (
        <AdminCard>
          <AdminTable headers={["Name", "Email", "Message", "Status", "Date", "Actions"]}>
            {items.map((f) => (
              <tr key={f.id} className="border-b border-[var(--divider)]">
                <td className="py-2 px-2 font-medium text-charcoal">{f.name || "Anonymous"}</td>
                ...
                <td className="py-2 px-2 text-charcoal max-w-[280px]">
                  <span className="line-clamp-2 block" title={f.message}>
                    {f.message}
                  </span>
                </td>
                <td className="py-2 px-2">
                  <StatusBadge status={f.status} variant={statusVariant(f.status)} />
                </td>
                <td className="py-2 px-2 text-secondary text-xs">
                  {new Date(f.created_at).toLocaleString()}
                </td>
                <td className="py-2 px-2">
                  <Link href={`/admin/feedback/${f.id}`} className="text-primary text-sm hover:underline">
                    View
                  </Link>
                </td>
              </tr>
        ...
        <AdminEmptyState message="No feedback submissions yet." />
      )}
    </div>
  );
}
