  ...
  if (sql) {
    try {
      const [postsRows, settingsRows] = await Promise.all([
        sql`SELECT id, slug, title, summary, seo_description, published_at, og_image_url, jlpt_level, tags, content_type FROM posts WHERE status = 'published' AND content_type IN ('blog', 'study_guide') ORDER BY published_at DESC LIMIT 200`,
