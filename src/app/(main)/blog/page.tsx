  ...
  if (sql) {
    try {
      const [postsRows, settingsRows] = await Promise.all([
        sql`SELECT id, slug, title, summary, seo_description, published_at, og_image_url, jlpt_level, tags, content_type, blog_category, author_name FROM posts WHERE status = 'published' AND content_surface IN ('blog_editorial', 'both') ORDER BY published_at DESC LIMIT 200`,
