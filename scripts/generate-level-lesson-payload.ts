...
async function ensureExercisePosts(levelCode: string, lessonId: string, lessonTitle: string) {
  ...
  const existing = (await sql`
    SELECT content_role FROM curriculum_lesson_content WHERE lesson_id = ${lessonId} AND content_role = 'exercise'
...
async function processLevel(levelCode: string, dryRun: boolean) {
  ...
  for (const r of lessonRows) {
    ...
    if (!dryRun) {
      const before = await sql`SELECT COUNT(*)::int AS c FROM lesson_blocks WHERE lesson_id = ${lessonId}` as { c: number }[];
      await ensureLessonBlocksMain(lessonId, lessonTitle);
      await ensureExercisePosts(levelCode, lessonId, lessonTitle);
      const after = await sql`SELECT COUNT(*)::int AS c FROM lesson_blocks WHERE lesson_id = ${lessonId}` as { c: number }[];
